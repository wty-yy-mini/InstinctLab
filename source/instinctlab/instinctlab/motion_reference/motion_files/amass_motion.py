...
# Joint order stored by the HUTT/unitree_rl_lab LAFAN1 G1 npz conversion pipeline.
HUTT_LAFAN1_G1_JOINT_NAMES = [
...
class AmassMotion(MotionBuffer):
    ...
    def _read_motion_file(self, motion_file_idx: int) -> MotionSequence:
        ...
        if filepath.endswith("poses.npz") or filepath.endswith("stageii.npz"):
            ...
            return self._read_hutt_lafan1_g1_motion_file(filepath)
        else:
    ...
    def _read_hutt_lafan1_g1_motion_file(self, filepath: str) -> MotionSequence:
        raw_data = np.load(filepath, mmap_mode="r", allow_pickle=True)
        required_keys = {"fps", "joint_pos", "body_pos_w", "body_quat_w"}
        missing_keys = required_keys.difference(raw_data.files)
        if missing_keys:
            raise ValueError(
                f"Unsupported .npz motion format: {filepath}. Missing keys for HUTT LAFAN1 G1: {missing_keys}"
            )

        framerate = float(np.asarray(raw_data["fps"]).reshape(-1)[0])
        joint_pos = torch.as_tensor(raw_data["joint_pos"], device=self.buffer_device, dtype=torch.float)
        root_trans = torch.as_tensor(raw_data["body_pos_w"][:, 0], device=self.buffer_device, dtype=torch.float)
        root_quat = torch.as_tensor(raw_data["body_quat_w"][:, 0], device=self.buffer_device, dtype=torch.float)

        if joint_pos.shape[-1] != len(HUTT_LAFAN1_G1_JOINT_NAMES):
            raise ValueError(
                f"Unsupported HUTT LAFAN1 G1 joint dimension in {filepath}: expected"
                f" {len(HUTT_LAFAN1_G1_JOINT_NAMES)}, got {joint_pos.shape[-1]}."
            )

        missing_joints = [j_name for j_name in self.isaac_joint_names if j_name not in HUTT_LAFAN1_G1_JOINT_NAMES]
        if missing_joints:
            raise ValueError(
                "Joint mapping failed for HUTT LAFAN1 G1 npz. Missing joints required by the articulation: "
                f"{missing_joints}. File: {filepath}"
            )
        source_joint_ids = [HUTT_LAFAN1_G1_JOINT_NAMES.index(j_name) for j_name in self.isaac_joint_names]
        joint_pos = joint_pos[:, source_joint_ids]
